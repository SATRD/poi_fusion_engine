...
public class CommonUtils {
	...
	private static JSONObject request(String url){
		...
		log = Logger.getLogger(org.upv.satrd.fic2.fe.connectors.citySDK.CommonUtils.class);
	...
	public static ArrayList<String> getPOICategoryList(String server){
	...
	public static JSONObject getPOIListBySourceAndCategory(String server, String source, String category, String bbox, String limit){
		...
		log.info("Using URLaccess for this category: "+url);
	...
	public static ArrayList<JSONObject> getZonePOIs(String server, String source, String category, String lat, String lon, String r, String sourceName, String percent){
		...
		log.info("Using URLaccess for potential matches: "+url);
			...
			for(int i=0; i<poiList.length(); i++){
				...
				if(sourceName != null){
					if(percent == null || !org.apache.commons.lang3.math.NumberUtils.isNumber(percent)){ 
						log.warn("Given a wrong value of similarity percentage. Using default value (60)..");
						percent = "60"; 
					}   //Default value
			    			
					int namePos = CommonUtils.getPropertyPos(poi,"name");
        			...
        			if(namePos != -1){
    					...
    					log.debug("Similarity between "+sourceName+" - "+name+" : "+per);
    					
    					if(per >= Double.parseDouble(percent)){
    						//We have a matching POI here. Add it
    						log.debug("Matching POI found");
    						result.add(poi);						
    					}
        			}
	...
	public static int getPropertyPos(JSONObject poi,String term){
		int result = -1;
		try{
	...
	public static boolean CheckPOIAlreadyInserted(Connection con, Integer sourceId, String originalRef, String projectedCategory){
						
		POISource poisource = POISource.getPOISourceClassBySourceIdAndOriginalRefAndPoiproxyAttribute(con, sourceId, originalRef, projectedCategory);
	...
	public static String getLatitude(JSONObject poi){
	...
	public static String getLongitude(JSONObject poi){
	...
	//This method scans a POI given as a JSON object and returns its longitude
	public static String getName(JSONObject poi){
	...
	public static POISource getPOISource(Connection con,JSONObject poi){
		POISource poisource;  
		JSONArray object;
	    JSONObject aux;
	    String source ="";
		...
		try{
			...
			Integer sourceId = (Source.getSourceClassByName(con, source)).getId();
			...
			log.error("getPOISource Error: "+ex.getMessage());
			System.out.println("getPOISource Error: "+ex.getMessage());
			return null;
		}
