...
public class Fusion {
	...
	public static FusionResult fusion(JSONObject poi1, JSONObject poi2, FusionRule fr){
		...
		try{
			...
			int namePos1 = CommonUtils.getPropertyPos(poi1,"name");
			String source1 = poi1.getJSONArray("label").getJSONObject(namePos1).getString("base");
			int namePos2 = CommonUtils.getPropertyPos(poi2,"name");
			...
			for(int j=0; j<fr.getName().size(); j++){
				if(fr.getName().get(j).equals(source1)){
					int namePos = CommonUtils.getPropertyPos(poi1,"name");
					object3.put(i,poi1.getJSONArray("label").getJSONObject(namePos));
					i++;
					log.info("Fusioning. Inserting name from source: "+source1);
					name_source = source1;
					break;				
				}
				if(fr.getName().get(j).equals(source2)){
					int namePos = CommonUtils.getPropertyPos(poi2,"name");
					object3.put(i,poi2.getJSONArray("label").getJSONObject(namePos));
					log.info("Fusioning. Inserting name from source: "+source2);
					name_source = source2;
					i++;
					break;					
				}
			...
			try{
				...
				object1 = null;
				log.warn("Fusioning. POI object from source "+source1+" does not have description. We'll take the description from the other POI independently of the FusionRule" );
			}
			try{
				object2 = poi2.getJSONArray("description");
			}catch (JSONException e){
				object2 = null;
				log.warn("Fusioning. POI object from source "+source2+" does not have description. We'll take the description from the other POI independently of the FusionRule" );
			}
					...
					for(int j=0; j<fr.getDescription().size(); j++){
						if(fr.getDescription().get(j).equals(source1)){
							if(poi1.getJSONArray("description").length() > 0){
								object3.put(i,poi1.getJSONArray("description").getJSONObject(i));
								log.info("Fusioning. Inserting description from source: "+source1);
								break;
							}
						}
						if(fr.getDescription().get(j).equals(source2)){
							if(poi2.getJSONArray("description").length() > 0){
								object3.put(i,poi2.getJSONArray("description").getJSONObject(i));
								log.info("Fusioning. Inserting description from source: "+source2);
								break;
							}
						}
					}
			...
			try{
				...
				object1 = null;
				log.warn("Fusioning. POI object from source "+source1+" does not have category." );
			}
			try{ 
				object2 = poi2.getJSONArray("category");
			}catch (JSONException e){
				object1 = null;
				log.warn("Fusioning. POI object from source "+source2+" does not have category." );
			}
			...
			try{
				...
				object1 = null;
				log.warn("Fusioning. POI object from source "+source1+" does not have link." );
			}
			try{ 
				...
				object2 = null;
				log.warn("Fusioning. POI object from source "+source2+" does not have link." );
			}
	...
	public static int save(Connection con, JSONObject poi, String category, String name_sourceName, String position_sourceName, ArrayList<org.upv.satrd.fic2.fe.db.POISource> poisourceArray ){
				
		Integer poiid = -1;		
		
		try{
			//Generate the POI object
			Double lat = new Double(CommonUtils.getLatitude(poi));
			Double lon = new Double(CommonUtils.getLongitude(poi));			
			String name = CommonUtils.getName(poi);
			Date date = CommonUtils.getFormatedDateAsDate();
			
			POI poi_object = new POI(name,lat,lon,date);   		
					
			poiid= POI.savePOI(con, poi_object);
			...
			if (poiid!=null){    			
			    ...
			    Category cat = Category.getCategoryClassByName(con,category);	
			    POICategory poicategory = new POICategory(poiid,cat.getId());
			    POICategory.savePOICategory(con, poicategory);
			    ...
			    for (int p=0;p<poisourceArray.size();p++){
			    	...
			    	org.upv.satrd.fic2.fe.db.POISource.savePOISource(con, poisource);				
			    }
			    ...
			    try{
					...
					object = null;					
					log.debug("Saving POI. The POI does not have any description. No label to insert in the POILabel table" );
				}
				...
				if (object !=null){		
						...
						try{
							...
							value = " ";
							log.debug("Saving POI. The POI has description, but no value field. It will be empty" );
						}
						...
						sourceId = Source.getSourceClassByName(con, source).getId();
						...
						try{
							...
							licenseId = License.getLicenseClassByName(con, license).getId();
					    }catch (JSONException e){
					    	license = "";
				...
				if (labeltype ==null){
					...
					name_labeltypeid = LabelType.saveLabelType(con, labeltype);					
				}else{
					name_labeltypeid = labeltype.getId();
				}
				sourceId = Source.getSourceClassByName(con, name_sourceName).getId();
				POILabel poilabel_aux = new POILabel(poiid, name_labeltypeid, name_sourceName, sourceId, null, null, updated);
				POILabel.savePOILabel(con,poilabel_aux);
				//position. Here we do not need to save the position (analog as with name)
				labeltype = LabelType.getLabelTypeClassByName(con, "position");
				...
				if (labeltype ==null){
					...
					position_labeltypeid = LabelType.saveLabelType(con, labeltype);					
				}else{
			    ...
			    try{
					...
					object = null;
					log.debug("Saving POI. Fusion object does not have any link. No label to insert in the POILabel table" );
				}
				if (object !=null){	
					...
					for(int i=0; i<object.length(); i++){							
							...
							type = "link";
							labeltype = LabelType.getLabelTypeClassByName(con, type);
							if (labeltype == null){
								labeltype = new LabelType(type);
								typeId = LabelType.saveLabelType(con, labeltype);
							}else{
								typeId = LabelType.getLabelTypeClassByName(con, type).getId();
							}
			    ...
			    try{
					...
					object = null;
					log.debug("Saving POI. Fusion object does not have any labels. No label to insert in the POILabel table" );
				}
				if (object !=null){							
					for(int i=0; i<object.length(); i++){
							...
							if (!type.equalsIgnoreCase("id")){
								...
								try{
									...
									value = " ";
									log.debug("Saving POI. The POI has a label, but no value field. It will be empty" );
								}								
								...
								try{
									license = (String)aux.get("license");									
									License license_aux = License.getLicenseClassByName(con, license);
			...
			System.out.println("Error saveCitySDKPOI: "+e.getMessage());
			log.error("Error saveCitySDKPOI: "+e.getMessage());
		}
		return poiid;
	}
